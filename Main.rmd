...
```{r,eval=FALSE}
rm(list = ls())
a <- install.packages("viridis", repos = "https://cloud.r-project.org")
```
...
devtools::install("dplyr") # Install dplyr
# Equals to:
...
```{r,eval=FALSE}
library(tidyverse) # to import tidyr, dplyr, readr and gglot
library(readr) # to import readr only
```
...
## *de novo* generation of data by `rnorm()`
...
Use `pnorm(val)` to calculate the Cumulative Distribution Function valur fot `val`.
...
for (i in seq(-10, 10, 0.1)) { a <- rbind(a, c(i, pnorm(i))) }
plot(a, type = "l", xlab = "X", ylab = "CDF(X)", main = "Cumulative Distribution Function for Normal Distributions")
```
